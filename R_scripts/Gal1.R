...
Gal1_data <- read.csv("Sc_Gal1p_BLAST_top_hits.csv", header = TRUE)
Gal1_data

x <- Gal1_data$Bit_score_1
y <- Gal1_data$Bit_score_2

# plot(Bit_score_1,Bit_score_2)
# xaxs and yaxs set to "i" to meet at zero
# limits for axes are set as well

#pdf("Sc_Gal1p_BLAST_top_hits.pdf")
plot(x,y, main="Gal1p top BLAST hit comparison", xlab="First_BLAST_hit_score", ylab="Second_BLAST_hit_score", xaxs="i", yaxs="i", xlim=c(0,1200), ylim=c(0,1200))
	#use identify function to get species names that correspond
	#to individual dots
	identify(x,y,labels=Gal1_data$Species, offset=0.5, cex=0.7)
