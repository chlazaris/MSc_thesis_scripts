...
reciprocal BLAST search based on RoundUp
and write the output to a file in a new
directory.
"""
...
def checkdir(d):
    ...
    try:
        ...
        raise IOException("Directory not found")
...
checkdir("Fungi_refseq_proteomes")
...
# get the corresponding proteome file
query_prot = fungi_species[query_prot_species]

# ask for subject proteome
subject_prot_species = raw_input("Please provide species name for subject proteome:\n")
subject_prot_species = subject_prot_species.rstrip()

# check if file exists
subject_prot = fungi_species[subject_prot_species]
#checkfile(subject_prot)
...
#keep part of ID file 
#for output file and directory
ids_parts = ids.split("_")
output_id_part = "_" + ids_parts[1]
...
output = output_query_part + "_vs_" + output_subject_part + output_id_part + settings_part + "_orthologs.txt"

# run the search and produce resulting file
os.system("rsd_search -q ./Fungi_refseq_proteomes/%s -s ./Fungi_refseq_proteomes/%s -o %s --ids %s --no-blast-cache --de %s %s " %(query_prot, subject_prot, output, ids, div, e_val))

#make a directory to save the resulting file
output_dir_name = output_query_part + "_vs_" + output_subject_part + output_id_part + settings_part + "_orthologs"
...
#and save in file
os.system("python report_results.py %s/%s" %(output_dir_name,output))

#run the script to create .csv file with results
